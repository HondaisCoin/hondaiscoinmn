...
*Setup instructions for a Gitian build of HondaisCoinMN Core using a Debian VM or physical system.*
...
- [Building HondaisCoinMN Core](#building-hondaiscoinmn-core)
...
Clone the git repositories for HondaisCoinMN Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hondaiscoinmn-official/hondaiscoinmn
```
...
in the HondaisCoinMN Core repository under 'Fetch and build inputs' to install sources which require
...
To build HondaisCoinMN Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the HondaisCoinMN Core repository.
...
```bash
    ...
    From https://github.com/hondaiscoinmn-official/hondaiscoinmn
...
and the hondaiscoinmn git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hondaiscoinmn/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/hondaiscoinmn.git
SIGPATH=/some/root/path/hondaiscoinmn-detached-sigs.git

./bin/gbuild --url hondaiscoinmn=${BTCPATH},signature=${SIGPATH} ../hondaiscoinmn/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hondaiscoinmn-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hondaiscoinmn-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hondaiscoinmn-osx-build.assert
```
...
[hondaiscoinmn/gitian.sigs](https://github.com/hondaiscoinmn-official/gitian.sigs/) repository, or if that's not possible to create a pull
request.
