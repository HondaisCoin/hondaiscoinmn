...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "hondaiscoinmn (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all HondaisCoinMN specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N HDCMN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
/** Sanity checks
 *  Ensure that HondaisCoinMN Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HondaisCoinMN Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize HondaisCoinMN Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single HondaisCoinMN Core process is using the data directory.
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HondaisCoinMN Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // ********************************************************* Step 11c: update block tip in HondaisCoinMN modules
    ...
    // ********************************************************* Step 11d: start hondaiscoinmn-ps-<smth> threads
