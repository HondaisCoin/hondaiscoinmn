...
// ***TODO*** ScanHash is not yet used in HondaisCoinMN
//
...
void static BitcoinMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HondaisCoinMNMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HondaisCoinMNMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HondaisCoinMNMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HondaisCoinMNMiner -- runtime error: %s\n", e.what());
        return;
    }
}
