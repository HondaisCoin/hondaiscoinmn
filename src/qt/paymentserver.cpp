...
//
// OSX-specific way of handling hondaiscoinmn: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom hondaiscoinmn addresses are not supported
        ...
        // HondaisCoinMN amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
